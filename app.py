...
st.caption(
    "Proof-of-Concept Emergency Triage & Diagnostic Decision Support Engine"
)
...
BACKEND_URL = st.secrets.get(
    "BACKEND_URL",
    os.getenv("BACKEND_URL", "https://aegismed-ai-engine-2.onrender.com"),
)
AEGIS_API_KEY = st.secrets.get(
    "AEGIS_API_KEY", os.getenv("AEGIS_API_KEY", "aegismed_secure_key_2026")
)
...
SAMPLES = {
    "Acute Stroke (CVA)": (
        "PATIENT EMERGENCY ADMISSION: 62-year-old male presenting with sudden"
        " right-sided facial drooping, slurred speech, and weakness in the"
        " right arm. BP: 195/110 mmHg. Onset: 90 minutes ago."
    ),
    "Benzodiazepine Overdose": (
        "EMERGENCY ADMISSION: 29-year-old female brought in unresponsive next"
        " to empty pill bottles of Alprazolam (Xanax). SpO2 86% on room air, RR"
        " 8/min (severe respiratory depression), HR 54 bpm, BP 88/50 mmHg."
    ),
    "Acute Myocardial Infarction": (
        "PATIENT ADMISSION NOTE: 55-year-old female presenting with severe"
        " retrosternal chest pain radiating to left jaw and shoulder, with"
        " diaphoretic sweating. BP 140/90 mmHg, HR 105 bpm. Trop-I elevated at"
        " 0.85 ng/mL."
    ),
    "Organophosphate Toxicity": (
        "EMERGENCY ROOM ASSESSMENT: 28-year-old agricultural worker with"
        " accidental pesticide exposure. Pinpoint pupils (miosis), profuse"
        " salivation, vomiting, wheezing. HR 48 bpm, BP 85/55 mmHg."
    ),
    "Septic Shock": (
        "EMERGENCY ADMISSION: 71-year-old female with high fever (39.4°C),"
        " confused and lethargic. HR 128, BP 82/48 mmHg. Lactate 4.8 mmol/L,"
        " WBC 22,000 /uL."
    ),
}

default_text = (
    SAMPLES.get(sample_choice, "") if sample_choice != "Custom Input" else ""
)
...
document_text = st.text_area(
    ...
    height=180,
    placeholder=(
        "Enter patient symptoms, vitals, lab values, or emergency presentation"
        " notes here..."
    ),
)
...
if st.button("🚀 Analyze & Generate Clinical Audit", type="primary"):
  if not document_text.strip():
    ...
    with st.spinner(
        "Querying ChromaDB vector database & running Gemini evaluation..."
    ):
      try:
        # Pass required authentication header to backend
        headers = {
        ...
        if response.status_code == 200:
          ...
          if isinstance(primary_diag, str):
            ...
            justification = primary_diag.get(
                "clinical_justification", "N/A"
            )
          ...
          if diff_diagnoses:
            ...
            for diff in diff_diagnoses:
              st.markdown(
                  f"- **{diff.get('condition_name')}**"
                  f" (*{diff.get('triage_level')}*):"
                  f" {diff.get('clinical_justification')}"
              )

          # Clinical Justification & Guideline
          st.markdown("### 📜 RAG Clinical Justification & Guideline")
          st.info(
              f"**Clinical Justification:** {justification}\n\n**Reference"
              f" Guideline:** `{ref_guideline}`"
          )
        ...
        st.error(
            f"Failed to connect to AegisMed API at `{BACKEND_URL}`. Error"
            f" details: {str(e)}"
        )